...
//   v5 – 2026-06-21
//          - LIVE INTEGRATION: PlaceholderTool replaced with the real
//            <iframe src="/mapper/index.html"> (Care Ecosystem Mapper deployed
//            to /mapper/ on the site). allow="microphone" added for in-tool
//            voice dictation.
//          - Modal chrome slimmed: removed the duplicate title header and the
//            footer (the mapper renders its own header inside the iframe). Kept a
//            slim "Return to survey" escape-hatch bar.
//          - toolDone listener unchanged — the mapper's close screen posts
//            {type:"toolDone"} to close the modal and unlock sections 1–11.
// =============================================================================
    ...
    return (
      ...
      <div className="fixed inset-0 z-50 flex flex-col" style={{ background: "rgba(0,0,0,0.75)" }}>
        <div className="flex flex-col flex-1 m-3 rounded-xl overflow-hidden bg-white shadow-2xl">
          ...
          {/* Slim escape-hatch bar. The mapper inside the iframe shows its own
              "Care Ecosystem Mapper" header, so we don't repeat a title here; we
              keep only an always-available way back to the survey, in case a
              participant wants to exit before finishing the mapper's own flow. */}
          <div
            className="flex items-center justify-end px-3 flex-shrink-0"
            style={{ background: "#0D3B38", height: "34px" }}
          >
            <button
              onClick={onClose}
              className="flex items-center gap-1.5 text-white font-semibold rounded-md transition-colors"
              style={{ background: "rgba(255,255,255,0.12)", border: "0.5px solid rgba(255,255,255,0.25)", fontSize: "11px", padding: "4px 10px" }}
            >
              <Check size={12} />
              Return to survey
            </button>
          ...
          {/*
            ── TOOL CONTENT ─────────────────────────────────────────────────────
            The real Care Ecosystem Mapper, deployed to /mapper/ on the site. It
            loads in this iframe; the handleIframeLoad + postMessage zoom and the
            toolDone listener above are already wired. The mapper auto-fits its own
            canvas, so the setZoom message is harmless if ignored.
  
            allow="microphone" is required for the in-tool voice dictation (the mic
            button in free-text fields). The participant still grants permission via
            the browser prompt; this attribute just permits the iframe to ask.
            ─────────────────────────────────────────────────────────────────── */}
          <iframe
            ref={iframeRef}
                  ...
                  return (
                    <button key={i} type="button" onClick={() => goTo(i)}
                      ...
                      className={`flex flex-col items-center gap-1 py-2 rounded-lg border transition-all ${
                        state === "active" ? "bg-teal-600 border-teal-600"
                        : state === "done" ? "bg-teal-50 border-teal-100 hover:bg-teal-100"
                        : "bg-gray-100 border-transparent hover:bg-teal-50"}`}>
                      <Icon size={16} className={state === "active" ? "text-white" : state === "done" ? "text-teal-600" : "text-gray-400"} />
                      {state === "active" && <span className="text-white text-xs leading-tight text-center">{label}</span>}
                    </button>
                  );
                })}
              </div>
